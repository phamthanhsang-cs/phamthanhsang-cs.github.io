...
"The attacks use a decoy document loaded with malicious macro code that downloads Cerber malware files to users’ machines. When the decoy document is opened, it drops a VBScript file (.vbs) with a random name in the format **“%APPDATA%\%RANDOM%.vbs.”**"
...
This question may not relate to the infected procedure of Cerber ransomware but gave beginning of the story why we8105desk got infected, the Wayne Enterprise employee grab some random USB and then insert that device into company workstation.

This question might trick us a lot ? since almost y'all will be think it should be the actual name of the USB device (e.g., "SanDisk Ultra" or "Kingston DataTraveler")...included me.

So, like question 23 (203), i was a bit lost to find the correct answer, guest what ? Yup, to find the answer i have to know what's actually happened on my machine when i insert a USB to my Laptop.
