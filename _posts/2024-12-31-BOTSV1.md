...
categories: [SIEM, SOC, Blue-team, CTF]
...
But it's kinda easy to look at the question and search for relevant data base on keyword:
```sql
index=botsv1 sourcetype=stream:dns we8105desk

///Do not forget to adjust the time range to 08/24/2016 to capture the relevant events.
```

There were approximately 10,000 DNS events recorded for `we8105desk` on 08/24/2016 - well, a lot to sift through. To narrow this down, I focused on filtering events with the following query types:
- query.type{} = "A"
- query.type{} = "AAAA"
...
```sql
index=botsv1 sourcetype=stream:dns query.type{}=A OR query.type{}=AAAA
| reverse
| table _time query
| dedup query
```
...
If you want to understand the full picture of how Cerber Ransomware operates, please take 10 minutes to read this detailed article: [Anatomy of a Ransomware Attack – Cerber Uses Steganography to Hide in Plain Sight.](https://www.netskope.com/blog/anatomy-ransomware-attack-cerber-uses-steganography-hide-plain-sight)
