...
- ~80,000 events tied to "we8105desk" on 08/24/2016.
- These events span 8 sourcetypes. 

Looking at the sourcetypes, which one catches your attention the most?

For me, the `nessus` sourcetype stands out as the most interesting. Why?
...
- Based on this, it’s reasonable to hypothesize that, in this scenario, the attacker used **[Nessus](https://www.tenable.com/products/nessus)** as their vulnerability scanning tool to find vulnerabilities from Wayne Enterprise employee' machine.
- Only 100 events generated from the sourcetype which is reduce our workloads on investigation.

Let's investigate the sourcetype and see could we find the answer base on our hypothesis.   

```sql
index=botsv1 sourcetype=nessus we8105desk
```
