...
When i first read, this question seems easy in first place BUT IT WAS NOT ! 

Started with down the scope by using `|metasearch index=botsv2 sourcetype=* MACLORY-AIR13 | fieldsummary` and thought i could get some useful informations especially with `stream:ldap` because it contains basic information about hosts.

But it was like 20 milions event data records and only `collectd` as the sourcetype (yeah ofc, its a MacBook and yeah Unix like system...)
