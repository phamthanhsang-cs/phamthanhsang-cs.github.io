...
Based on [MITRE ATT&CK](https://attack.mitre.org/techniques/T1491/002/), the adversary used T1491.001 - Defacement: External Defacement to defaced Batman's website(71.39.18.126). But....how the attacker did that ? So, we could assume that Po1s0n1vy was able to fire a phishing campaign successfully and then from that compromised account / machine,...they did web scan, exploited the jooma CMS vulnerability and defaced the website. 
...
- `filename="3791.exe" ` which is the malicious excutable file
- `dtype="Virus"`
...
Aight, let's get back to the question and combine with my previous hypotheosis but in other situation where Po1s0n1vy had failed to compromise the web server. Now, they might use the `lillian.rose@po1s0n1vy.com` to send phishing emails to Wayne Corp' employees, and trying to exploit the system base on "human error". We knew that email tied to `23.22.63.114`, stumbled around some popular OSINTs: Virus Total, AlienVault, abuse.ch but got no clue. I'd decided to use ChatGPT on this question with prompt:`which osint tool should i use to detect previous file was used on a domain name or an IP address` and got these suggesstion:
- VirusTotal
- Hybrid Analysis
- MalwareBazaar
- Recorded Future
...
- Start with VirusTotal to gather initial insights.
- Cross-check with Hybrid Analysis or MalwareBazaar for sandbox reports.
- Use MISP/OpenCTI if you maintain private threat intelligence data.
- Enrich results with AlienVault OTX and ThreatMiner for community-driven insights.

And we did Virus Total and AlientVault researched. Hybrid Analysis is for sandbox report which is not suitable for this situation, MalwareBazaar is one of the threat database in abuse.ch. MISP / OpenCTI is like TIP (Threat Intelligence Platform) and usually use for private TI data for organization (Please take a look into my [SOCIMP](https://github.com/phamthanhsang-cs/SOC-in-my-Pocket) - i'm using both of those as Cyber Threat Intelligent for the project), so we should go to [ThreatMiner](https://www.threatminer.org/) then. 
